...
4. Configure Gemini API
    ...
    Or update in code (line in cell 3):
...
Step 2: Run the Notebook
    ...
    Open RAG_Implimentation.ipynb in Jupyter or VS Code
    Run cells in order:
...
Solutions:
    ...
    The system will create a new FAISS index automatically
    First run will take longer (5-10 minutes)
    Subsequent runs will load the cached index (faster)
...
Solutions:
    ...
    Verify Java files are in Group1/, Group2/, Group3/ directories
    Check file extensions are .java
    Update the path in Cell 9 if needed:
...
Performance Tips
    ...
    First Load: Takes 5-15 minutes (building embeddings)
    Subsequent Loads: 30-60 seconds (loading cached index)
    Query Response: 10-30 seconds (API call + retrieval)
...
Next Steps
    ...
    Customize the RAG prompt in Cell 18 for specific use cases
    Adjust chunk size and overlap in Cell 11 for better results
    Add more Java files to the Group directories
    Fine-tune the retriever k parameter for more/fewer results
